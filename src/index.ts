import './styles/global.css';
import { restoreThemePreference } from './theme/theme-preferences.js';

// Export components
export { IndApplication }  from './components/application/application-component.js';
export { IndButton }       from './components/button/button-component.js';
export { IndCard }         from './components/card/card-component.js';
export { IndKnob }         from './components/knob/knob-component.js';
export { IndMenu }         from './components/menu/menu-component.js';
export { IndMenubar }      from './components/menubar/menubar-component.js';
export { IndSidebar }      from './components/sidebar/sidebar-component.js';
export { IndTile }         from './components/tile/tile-component.js';
export { IndToggleSwitch } from './components/toggle-switch/toggle-switch-component.js';

// Export icons
export * from "./icons/index.js";

// Restore theme preference
restoreThemePreference();